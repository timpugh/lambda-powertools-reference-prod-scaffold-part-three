...
**Flaky jsii `KeyError` on `cloud_assembly_schema.MetadataEntry` under coverage** — the docs workflow's coverage-badge step (and `make coverage` / `make coverage-badge`) runs the CDK assertion suite under `coverage.py`, and intermittently failed with `KeyError: 'aws-cdk-lib.cloud_assembly_schema.MetadataEntry'` raised inside jsii's Python `_reference_map`. `MetadataEntry` is a jsii **struct** (`@jsii.data_type`), which jsii is designed to pass *by value* ([aws/jsii#376](https://github.com/aws/jsii/pull/376)); the failure is the kernel occasionally returning it as an opaque object **reference** whose declared `interfaces` jsii then can't resolve (structs aren't in the interface registry it looks them up in). It surfaces **only** under coverage's tracer timing — the `cdk-check` gate runs the same tests *without* `--cov` and never hits it, which is why merges stayed green while only the docs deploy flaked. This is an instance of the broader "AWS CDK under pytest" jsii-kernel flakiness where a suite passes in isolation but fails in a full single-process run ([aws/aws-cdk#3759](https://github.com/aws/aws-cdk/issues/3759)). **Mitigation:** the `_cdk-cov-retry` Make target retries the coverage run up to three times (clearing partial data between attempts), so a flaky failure recovers while a genuinely broken test fails every attempt and still fails the build. It is not a correctness gate — the authoritative CDK test gate is `cdk-check`, which is unaffected.
