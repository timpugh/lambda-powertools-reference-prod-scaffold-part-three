...
**Push-to-deploy requires the explicit V2 trigger — don't remove it.** CDK synthesizes this as a **V2** pipeline with no `Triggers` block, and V2 pipelines do *not* inherit V1's connection-based change detection. Live-proven: pushes to `main` started **zero** executions until `_declare_push_trigger` (`pipeline_stack.py`) added the explicit git push trigger — a gap that hid for a while because early pushes happened to coincide with `restart_execution_on_update` restarts from workstation deploys, which *look* like push-triggered runs. `test_push_trigger_is_explicitly_declared` pins the block; a pipeline that only ever runs when someone deploys or updates it is CI theater, so treat that test failing as a broken pipeline even though every stage still works when started by hand.
