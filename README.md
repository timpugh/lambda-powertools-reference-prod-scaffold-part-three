...
The recipe that actually rotates it: **add a new `Secret` construct, in two deploys — not a rename.** Renaming the existing construct id (`OriginVerifySecret` → `OriginVerifySecretV2`) looks like the obvious move, but it bounces off CloudFormation's in-use-export trap (the same one the `api_url` export needed a temporary retention for during the same-origin migration — since removed once every environment had deployed past it; `TODO.md`'s Infrastructure section records that arc): a rename makes CloudFormation replace the secret, the *old* secret's ARN is exported for the frontend's dynamic reference, `cdk deploy '**'` updates the backend stack first, and CloudFormation refuses to delete an export while another stack still imports it — the deploy fails on first use.
...
| WAF BlockedRequests (regional ACL) | > 100 / 5 min | `BackendApp._attach_waf_alarms` | Addressed by the WebACL's **name** (its resource name, e.g. `f"{stack_name}-api"`) — not its `VisibilityConfig.MetricName` — no cross-stack/cross-region reference needed. Live-verified against a deployed environment: the CloudWatch `WebACL` dimension carries the ACL's name; the ACL-level `VisibilityConfig.MetricName` only ever appears as a `Rule` dimension value, on default-action metrics — which is what makes the AWS docs phrase "the metric name of the WebACL" misleading |
| WAF BlockedRequests (CloudFront ACL) | > 100 / 5 min | `BackendApp._attach_waf_alarms` | Same by-name addressing (the ACL's name, e.g. `f"{waf_stack_name}-cf"`); the CloudFront-scope `WebACL` dimension is the one protected-resource type that omits `Region`, and CloudFront WAF metrics only publish in `us-east-1` — this alarm is only created when the deployed stack **is** in `us-east-1`. It's also a by-name metric with `treat_missing_data=NOT_BREACHING`, so a wrong `WebACL` dimension value never fires and shows **no error anywhere** — verify on a live deploy that the metric actually reports data (same verification posture as the spend budget below) |
...
**The one true vampire — the registered domain.** When you add a custom domain (part-three's arc), `cdk destroy` reaches the alias records and ACM certs (free) that live *inside* the zone, but two things sit outside every stack: the **hosted zone** ($0.50/mo — an account prerequisite by design, see the "Custom domain" decisions) and the **domain registration itself** — an annual, auto-renewing charge living in Route 53 Domains *outside CloudFormation*, owned by no stack. The zone has a zero-cost answer: **`make hibernate-dns`** deletes it when parking the project long-term (recurring drops to $0.00/month), and **`make wake-dns`** recreates it, re-points the registration's name servers, and rewrites `HOSTED_ZONE_ID` in `./.env` on resume. The registration survives everything by definition — it IS the domain. Manage it as a conscious decision: disable auto-renew when you're truly done, otherwise leave it and know it's the single line item that outlives `make audit-account`'s "CLEAN". `audit-account` always prints registered domains with their auto-renew status for exactly this reason.
...
**Export-retention interaction — resolved, pattern remains.** The pipeline's first prod deploy satisfied the "every environment has deployed once" condition, and `backend_stack.py`'s temporary `api_url` `export_value()` retention has been removed (verified removable first: `aws cloudformation list-imports` on that export was empty in both environments — see [TODO.md](TODO.md#infrastructure)). The two-deploy pattern itself still applies any time a change would replace an exported, cross-stack-consumed resource — the standing case is the `OriginVerifySecret` rotation recipe above, which keeps its retention *step* because every future rotation re-creates the in-use-export situation.

**Rollback is demonstrated, and recovery is just the next push.** Both halves of the canary machinery have been exercised against live prod: a clean shift (`CANARY_10PERCENT_5MINUTES` promoting to 100% with zero caller-visible errors) and a forced failure — the canary alias-errors alarm driven to `ALARM` mid-bake, CodeDeploy stopping the deployment autonomously (`ALARM_ACTIVE`) and reverting the `live` alias to the prior version, with every request through deploy+poison+rollback answering 200. Know the aftermath shape: the pipeline run ends with the prod Backend action **Failed** and the stack in `UPDATE_ROLLBACK_COMPLETE` — both are the *correct* record of a refused deployment, not damage. Recovery requires no console surgery and no special commands: `UPDATE_ROLLBACK_COMPLETE` is a fully updatable state, so the next push's run simply deploys the (still-in-`main`) change again through a fresh canary. This paragraph's own commit is that recovery push for the drill above.
...
**Push-to-deploy requires the explicit V2 trigger — as a matched pair, don't break either half.** CDK synthesizes this as a **V2** pipeline with no `Triggers` block, and V2 pipelines do *not* inherit V1's connection-based change detection. Two live-proven halves: pushes to `main` started **zero** executions until `_declare_push_trigger` (`pipeline_stack.py`) added the explicit git push trigger, **and** the trigger alone still wasn't enough — the source action's legacy `DetectChanges` flag had to be set `false` too (`trigger_on_push=False` on `CodePipelineSource.connection`), because the legacy change-detection mode and the V2 trigger are mutually exclusive routing modes and leaving the flag `true` suppressed `WebhookV2` registration entirely. The gap hid for a while because pushes kept coinciding with `restart_execution_on_update` restarts from workstation deploys, which *look* like push-triggered runs — CloudTrail (`Username: AWSCloudFormation` on every `StartPipelineExecution`) was the discriminator, and a genuine push start shows trigger type `WebhookV2`. `test_push_trigger_is_explicitly_declared` pins the trigger block and `test_source_is_the_codeconnections_repo` pins `DetectChanges: false`; a pipeline that only ever runs when someone deploys or updates it is CI theater, so treat either test failing as a broken pipeline even though every stage still works when started by hand. **And the third half lives on GitHub, outside anything CDK can pin**: the AWS Connector for GitHub app must be **installed** on the account (github.com/settings/installations) with this repo in its repository-access list. An app that is merely *authorized* (the OAuth grant every console handshake produces) reads a **public** repo perfectly — Source succeeds, handshakes complete, everything looks healthy — while delivering **zero** push events, because event delivery is a function of the installation, not the authorization. Live-proven: two fresh connections and fully correct AWS-side config produced nothing; the first post-install push started a `WebhookV2` execution in 90 seconds. When diagnosing, two discriminators cut through the noise: CloudTrail's `Username` on `StartPipelineExecution` events (`AWSCloudFormation` = a `restart_execution_on_update` restart masquerading as a push start) and the execution's `trigger.triggerType` (only `WebhookV2` is a genuine push start).

**Push before (or with) any workstation `make deploy-pipeline` — never after.** Live-proven failure mode: a workstation deploy of a pipeline-definition change *itself* restarts an execution (the `restart_execution_on_update` above), and that restarted run sources whatever `main` holds at that moment. If your change isn't pushed yet, the restarted run's SelfMutate stage happily re-imposes the old definition — silently reverting the update you just deployed — and the run triggered by your eventual push then fails against the reverted infrastructure. The safe order is always: commit → push → `make deploy-pipeline` (when a workstation deploy is needed at all; definition changes that don't have to fix a broken Synth can just ride self-mutation with no workstation deploy).

**The permissions boundary is now load-bearing for every deploy, not just the pipeline's.** `AppStage` and `PipelineStack` both pass `permissions_boundary=cdk.PermissionsBoundary.from_name("cdk-scaffold-boundary")` unconditionally, so `make bootstrap-boundary` (One-time setup, step 1) is a prerequisite before `make deploy`/`make deploy ENV=<name>` too — a role that references a missing boundary policy fails deploy with an IAM error. A boundary-blocked action fails at **deploy or runtime**, not at synth: `cdk synth` and cdk-nag have no visibility into the boundary policy's allow-list, so a permission the app needs but the boundary denies surfaces mid-deploy or the first time the runtime role tries the call. `--custom-permissions-boundary` only attaches the boundary to the bootstrap stack's `CloudFormationExecutionRole` — the deploy/file-publishing/image-publishing/lookup roles stay unbounded, same as stock `cdk bootstrap`. **Documented residuals:** the boundary denies creating unbounded principals and attaching/inlining policies onto them, but a bounded principal can still reach *pre-existing* unbounded principals through paths a boundary condition can't close — `iam:UpdateAssumeRolePolicy` on an unbounded role, group-policy edits (`iam:AttachGroupPolicy`/`iam:PutGroupPolicy` + `iam:AddUserToGroup`; boundaries never apply to groups), and credential minting on an existing unbounded user (`iam:CreateAccessKey`/`iam:CreateLoginProfile`). Nil in practice for this single-account solo-dev scaffold (no IAM users or groups exist); pair with org-level SCPs for defense in depth if these residuals matter for your account.

### The PromoteToProd gate — right-sized today, designed to be removed

The `ManualApprovalStep` between the integration-tested `dev` deploy and prod is a **maturity stage, not the end state**. Both halves of that sentence matter:

**Why it's the right call today.** A first-generation pipeline, a solo operator, and automated signals whose trustworthiness hasn't been demonstrated yet are exactly the conditions where AWS's own guidance endorses an approval action — and regulated environments keep one permanently for the audit trail. While the gate exists, operate it honestly: approve on **evidence**, not vibes (the ladder already enforces the biggest piece — an approval can't even be requested until the live integration suite passed against `dev`); write the *why* into the approval comment, since that's the audit record; scope who may approve via IAM (`codepipeline:PutApprovalResult` on this pipeline's Prod stage); and know that an untouched approval **auto-fails after 7 days** — a stale gate rejects itself rather than shipping stale code.

**Why it shouldn't survive forever.** The industry's high-maturity bar — [DORA](https://dora.dev/)'s elite tier and AWS's own practice per the Builders' Library ["Automating safe, hands-off deployments"](https://aws.amazon.com/builders-library/automating-safe-hands-off-deployments/) — has **no human gate at all**: promotion is decided by automated tests, canary traffic-shifting with alarm-driven rollback, and bake time. DORA's research finding is blunt: heavyweight human change-approval correlates with *worse* delivery performance and *no* reduction in change-failure rate — after enough green runs, the human clicking Approve has strictly less information than the alarms do, and the gate decays into a rubber stamp that only adds lead time.

**The graduation path is already built.** This pipeline carries the hands-off machinery today: the integration suite gates `dev`→prod automatically, prod code deploys shift through CodeDeploy's `CANARY_10PERCENT_5MINUTES` with automatic rollback on the canary alias-errors alarm, and the AppConfig rollback monitor is an opt-in flag away. Once a few production canaries have behaved — and ideally one has *demonstrated* rollback — removing the gate is deleting the `ManualApprovalStep` line in `infrastructure/pipeline_stack.py` (plus its stage-order test assertion): the pipeline becomes fully continuous with nothing else to build.
...
**What stays human, in total:** the purchase decision itself, possibly that one ICANN email click, and the once-per-park `hibernate-dns`/`wake-dns` commands if you use the zero-idle-cost lifecycle. Everything downstream — certificates, validation, edges, records, teardown of everything except the registration — rides the pipeline or a single Make target.
