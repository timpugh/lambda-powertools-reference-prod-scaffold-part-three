...
Step 1 deploys [`infrastructure/bootstrap/cdk-scaffold-boundary.json`](infrastructure/bootstrap/cdk-scaffold-boundary.json) — a standalone CloudFormation template, not part of the CDK app — as the `cdk-scaffold-boundary` managed policy. Commit and push the `code_connection_arn` from step 3 to `main` **before** (or together with) step 4's birth deploy: the pipeline's own Synth step runs bare `npx cdk synth -c pipeline=true` from the GitHub checkout, reading `cdk.json` as pushed — `make deploy-pipeline CONN=<arn>` only supplies the ARN to this one workstation-side `cdk deploy` and does not affect what the pipeline synths with on its first (or any) run, so an unpushed ARN fails the pipeline's first Synth even after a successful birth deploy. Step 4 is otherwise a one-time `cdk deploy ServerlessAppPipeline -c pipeline=true`; after it, the pipeline self-mutates from GitHub `main` on every push, so rerunning the target is only needed if the pipeline stack is ever deleted.
