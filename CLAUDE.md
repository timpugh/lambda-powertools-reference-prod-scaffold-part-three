...
Three SSE-S3 log-sink buckets share `nag_utils.create_sse_s3_log_bucket` (block-all, SSL, no versioning, lifecycle expiry, standard log-bucket suppressions): the frontend **access-log** bucket, the **CloudTrail-logs** bucket (audit), and the **WAF-logs** buckets. WAF logs go to S3, not CloudWatch — cheaper long-term retention, queryable via Athena (a WAF Glue/Athena table is a TODO). There are **two** WAF log buckets (the WAF→S3 destination must be in the ACL's region): one in `HelloWorldWafStack` (us-east-1, CloudFront WebACL) and one in the backend stack (`_attach_regional_waf`, target region) — both via `create_waf_logs_bucket` (`aws-waf-logs-{account}-{hash}-{suffix}`, AWS forces the `aws-waf-logs-` prefix).
...
## Deployment safety: canary Lambda + AppConfig rollback (env-gated)

Two progressive-delivery paths with automatic, alarm-driven rollback live in `hello_world/hello_world_app.py`, both gated by `is_production_env` (gradual in prod, fast in dev) — the machinery exists in both shapes, only the rollout speed differs.
...
- **Config (`_attach_appconfig_rollback_monitor`)**: the AppConfig deployment strategy is gradual+bake in prod / all-at-once in dev, and the environment **monitor** auto-rolls-back on a CloudWatch alarm. The monitored signal is the handler's `FeatureFlagEvaluationFailure` EMF metric, **not** Lambda errors — a bad flag config is caught by the handler's fallback and returns 200, so it produces no Lambda error; the custom metric is the only signal.

**Acyclicity constraint (don't regress this):** the AppConfig environment references the rollback alarm (monitors), and the Lambda's role references the environment (the `appconfig:GetLatestConfiguration` grant is scoped to the env ARN). So the rollback alarm must **not** transitively reference the Lambda, or `Template.from_stack` fails with a dependency cycle (note: the CLI `cdk synth` does *not* catch this cycle — only the in-process assertion does, so `make test-cdk` is the gate here). That's why the alarm watches a **by-name** metric (`cloudwatch.Metric(namespace="HelloWorld", metric_name="FeatureFlagEvaluationFailure", ...)`) instead of `self.function.metric_errors()` — a by-name metric carries no CDK dependency on the function. The canary alias-errors alarm *can* reference the alias because nothing the environment depends on references that alarm.

The two deployment-control alarms carry **no SNS action** (CodeDeploy/AppConfig poll their state) and suppress `CloudWatchAlarmAction` (NIST/HIPAA) in all environments — distinct from the MonitoringFacade operational alarms, which route to SNS in prod.
