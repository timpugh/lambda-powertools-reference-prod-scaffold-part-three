...
- [x] **Bytes-scanned-per-query data usage control** — `bytes_scanned_cutoff_per_query=1 GiB` is set on the access-logs workgroup in [hello_world/hello_world_frontend_stack.py](hello_world/hello_world_frontend_stack.py); Athena cancels any query that would exceed it, capping per-query cost against a forgotten `WHERE`. Generous for these log tables at 7-day retention — raise it in a fork with larger log volumes.
